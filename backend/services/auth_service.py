...
class AuthService:
    def table_columns(self, cursor, table: str) -> set[str]:
        cursor.execute(
    ...
    def ensure_columns(self, cursor, table: str, columns: dict[str, str]):
        existing = self.table_columns(cursor, table)
        for name, ddl in columns.items():
            if name not in existing:
                cursor.execute(f"ALTER TABLE {table} ADD COLUMN {name} {ddl}")

    def insert_tenant(self, cursor, tenant_id: str, name: str, created_at: str):
        cols = self.table_columns(cursor, "tenants")
        if "uuid" in cols:
            cursor.execute(
    ...
    def init_db(self):
        ...
        """)
        self.ensure_columns(cursor, "tenants", {
            "tenant_id": "TEXT",
            "tenant_uuid": "TEXT",
            "created_at": "TEXT"
        })
        self.ensure_columns(cursor, "users", {
            "tenant_id": "TEXT",
            "tenant_uuid": "TEXT",
            "invited_at": "TEXT",
            "last_login_at": "TEXT",
        ...
        for table in ("sources", "destinations", "connections", "logs"):
            ...
            """)
            self.ensure_columns(cursor, table, {"tenant_id": "TEXT"})
        conn.commit()
        conn.close()
    ...
    def seed_data(self):
        ...
        if not cursor.fetchone():
            self.insert_tenant(cursor, tenant_id, "System Workspace", now)
        cursor.execute("SELECT tenant_id FROM tenants WHERE name = ?", ("System Workspace",))
    ...
    def login_user(self, conn, username: str, password: str, tenant: str) -> Dict[str, Any]:
        cursor = conn.cursor()
        if tenant:
        ...
        if requires_reset:
            reset_token = self.create_token(user_row["username"], user_row["email"], tenant_id, user_row["role"], "first_login_reset", RESET_TOKEN_EXPIRE_MINUTES)
            raise HTTPException(
                status_code=428,
                detail={
        ...
        conn.commit()
        access_token = self.create_token(user_row["username"], user_row["email"], tenant_id, user_row["role"], "access", ACCESS_TOKEN_EXPIRE_MINUTES)
        return {
    ...
    def register_user(self, conn, tenant: str, username: str, email: str, password: str) -> Dict[str, Any]:
        cursor = conn.cursor()
        ...
        tenant_id = str(uuid.uuid4())
        now = datetime.utcnow().isoformat()
        self.insert_tenant(cursor, tenant_id, tenant, now)
        cursor.execute(
            "INSERT INTO users (email, username, password, tenant_id, tenant_uuid, role, is_first_login, last_login_at) VALUES (?, ?, ?, ?, ?, ?, 0, ?)",
            (email, username, pwd_context.hash(password), tenant_id, tenant_id, "Tenant_Admin", now)
        )
        conn.commit()
        access_token = self.create_token(username, email, tenant_id, "Tenant_Admin", "access", ACCESS_TOKEN_EXPIRE_MINUTES)
        return {
            "token_key": "access_token",
            "token": access_token,
            "tenant_id": tenant_id,
    ...
    async def invite_teammate(self, conn, email: str, name: str, role: str, claims: dict) -> Dict[str, Any]:
    ...
    def reset_user_password(self, conn, new_password: str, claims: dict) -> Dict[str, Any]:
        ...
        tenant_row = cursor.fetchone()
        conn.commit()
        access_token = self.create_token(claims["sub"], email, tenant_id, claims["role"], "access", ACCESS_TOKEN_EXPIRE_MINUTES)
        return {
            "token_key": "access_token",
            "token": access_token,
            "tenant_id": tenant_id,
    ...
    def authenticate_google(self, conn, email: str, name: str, google_id: str) -> Dict[str, Any]:
        cursor = conn.cursor()
        ...
        if user_row:
            ...
            now = datetime.utcnow().isoformat()
            self.insert_tenant(cursor, tenant_id, tenant_name, now)
            cursor.execute(
            ...
            role = "Tenant_Admin"
        conn.commit()
        access_token = self.create_token(username, email, tenant_id, role, "access", ACCESS_TOKEN_EXPIRE_MINUTES)
        return {
            "token_key": "access_token",
            "token": access_token,
            "tenant_id": tenant_id,
    ...
    def get_workspace_users(self, conn, tenant_id: str) -> List[Dict[str, Any]]:
        cursor = conn.cursor()
        cursor.execute(
